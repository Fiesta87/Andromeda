...
namespace Carbon
{
	...
	{
		return fabs(X - other.X) < Carbon::epsilon && fabs(Y - other.Y) < Carbon::epsilon && fabs(Z - other.Z) < Carbon::epsilon;
	}
	...
	{
		return std::acosf(Vector3::Dot(a, b) / (a.Magnitude() * b.Magnitude())) * Carbon::rad_to_deg;
	}
