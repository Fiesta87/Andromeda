...
namespace Carbon
{
	...
	Vector3 Quaternion::GetEulerAnglesRad() const
	{
		...
		if (fabs(fabs(XYplusZW) - 0.5f) < Carbon::epsilon)
		{
	...
	{
		return GetEulerAnglesRad() * Carbon::rad_to_deg;
	}
	...
	{
		SetEulerAnglesRad(x * Carbon::deg_to_rad, y * Carbon::deg_to_rad, z * Carbon::deg_to_rad);
	}


	void Quaternion::SetEulerAnglesDeg(const Vector3& eulerAngles)
	{
		SetEulerAnglesRad(eulerAngles.X * Carbon::deg_to_rad, eulerAngles.Y * Carbon::deg_to_rad, eulerAngles.Z * Carbon::deg_to_rad);
	}
	...
	{
		return fabs(X - other.X) < Carbon::epsilon && fabs(Y - other.Y) < Carbon::epsilon && fabs(Z - other.Z) < Carbon::epsilon && fabs(W - other.W) < Carbon::epsilon;
	}
	...
	{
		if (Vector3::Distance(from, to) <= Carbon::epsilon)	return Quaternion::IDENTITY;

		if (Vector3::Distance(from, -to) <= Carbon::epsilon) return Quaternion(0.0f, 1.0f, 0.0f, 0.0f); // 180° rotation on Y (axis doesn't matter, but UP seems more natural)
	...
	Quaternion Quaternion::FromEulerAnglesDeg(const Vector3& eulerAngles)
	{
		...
		q.SetEulerAnglesRad(eulerAngles.X * Carbon::deg_to_rad, eulerAngles.Y * Carbon::deg_to_rad, eulerAngles.Z * Carbon::deg_to_rad);
	...
	Quaternion Quaternion::FromEulerAnglesDeg(float x, float y, float z)
	{
		...
		q.SetEulerAnglesRad(x * Carbon::deg_to_rad, y * Carbon::deg_to_rad, z * Carbon::deg_to_rad);
